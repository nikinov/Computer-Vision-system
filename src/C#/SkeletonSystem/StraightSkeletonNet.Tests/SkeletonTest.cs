...
namespace StraightSkeletonNet.Tests
{
    ...
    public class SkeletonTest
    {
        ...
        [Test]
        public void SkeletonTest_hole_3()
        {
            var inner = new List<Vector2d>
            {
                new Vector2d(1, 1),
            ...
            var expected = new List<Vector2d>
            {
                ...
                new Vector2d(6.5, .5),
                new Vector2d(6.5, 7.5),
            };

            expected.AddRange(outer);
            expected.AddRange(inner);
            expected.AddRange(inner2);
            expected.AddRange(inner3);

            List<Vector2d> allPoints = new List<Vector2d>();
            allPoints.AddRange(outer);
            allPoints.AddRange(inner);
            allPoints.AddRange(inner2);
            allPoints.AddRange(inner3);
            ...
            foreach (var edge in sk.Edges)
            {
                if (edge.Polygon.Count == 4)
                {
                    List<Vector2d> line = new List<Vector2d>();
                    quad += 1;
                    foreach (var pos in edge.Polygon)
                    {
                    ...
                    foreach (var lin in lines)
                        if (lin[0] == line[1] && lin[1] == line[0])
                            pass = false;

                    if (pass)
                        lines.Add(line);
                }
                else if (edge.Polygon.Count == 3)
                {
                    tri += 1;
                }
                else
